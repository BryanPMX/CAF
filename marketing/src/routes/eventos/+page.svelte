        ...
        {#each data.events as event, i}
          ...
          >
            <div class="flex flex-col md:flex-row">
              <!-- Date badge -->
              <div class="md:w-28 bg-gradient-to-br from-primary-600 to-accent-600 text-white flex flex-col items-center justify-center py-4 md:py-6">
                <span class="text-3xl font-bold">{event.date.day}</span>
                <span class="text-sm font-semibold tracking-wider">{event.date.month}</span>
                <span class="text-xs opacity-75">{event.date.year}</span>
              </div>
